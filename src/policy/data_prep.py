...
directory is decoded exactly once into a shard:
    ...
    <output-root>/_shards/<task>/
        ...
        state.npy       float32 [N, 7]       measured EEF pose + gripper
        action.npy      float32 [N, 7]       commanded EEF pose + gripper target
...
BUNDLE_SPECS: dict[str, tuple[str, ...]] = {
    "pick-can-all": ("pick-can-1", "pick-can-2"),
    "pick-scissors-all": ("pick-scissors-1", "pick-scissors-2"),
    "move-bar-all": ("move-bar-1", "move-bar-2"),
}
