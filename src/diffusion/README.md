...
1. an unconditional video DDPM at **56×56**;
2. a conditional video DDPM that super-resolves the generated clip from
   **56×56 to 224×224**.
...
every clip. Each original 480×640 frame is center-cropped first:
...
Defaults use 16-frame clips, 1,000 diffusion time steps, a cosine noise
schedule, EMA weights, and Min-SNR weighting. The 3D U-Net downsamples only the
spatial axes; temporal 3×3×3 convolutions retain cross-frame coherence. The
...
python src/diffusion/train.py --stage base --device cpu \
  ...
  --base-channels 8 --channel-multipliers 1 2 --clip-length 2 \
...
Validation holds out complete episode videos, so overlapping clips from an
episode never appear in both training and validation.
...
Sampling uses 50 DDIM steps per stage by default and EMA weights. Keep
`--num-frames` equal to the training clip length for the most reliable result.
The network is temporally fully convolutional, so other lengths are accepted,
but substantially longer videos were not seen as one unit during training.
