...
def checkpoint_panel() -> PolicySettings | None:
    ...
    if bool(config.get("goal_conditioned")):
        ...
        goal_help = (
            "A picture of where the episode should be heading, from this camera. Training took "
            f"frame {config.get('goal_frame_index', 2)} of {config.get('goal_frames', 4)} sampled "
            "evenly across each episode, which is the frame a video model generates - the "
            "goal-conditioned page produces one for you."
            if selection == "uniform4"
            else "A photo of the finished task, from this camera. Training drew the goal from the "
            f"last {config.get('goal_window', 10)} frames of each episode, so a frame grabbed "
            "from a successful rollout is exactly the right thing to point at."
        )
        typed = st.text_input("Goal image", value="", disabled=driving, help=goal_help).strip()
...
def outcome_panel(outcome: str | None) -> None:
    """Judge the finished episode: did the policy do the task or not?

    The verdict is the measurement an evaluation rollout exists to produce - a
    success rate is a count of these - and it is also the preference label
    ``src/finetune`` turns into DPO pairs and ``src/guidance`` trains its
    classifier on. It has to be recorded while the operator still remembers what
    the arm actually did, so Save stays disabled until one of the two is picked.
    """
    ...
    if outcome is None:
        ...
        return
    label = "success" if outcome == "success" else "failed"
    st.caption(f"Marked **{label}**; it is written to the episode's meta.json as `outcome`.")
    if st.button("↺ Clear the judgement", width="stretch"):
        runtime.set_rollout_outcome(None)
        st.rerun(scope="fragment")


@st.cache_data(ttl=5.0, show_spinner=False)
def _outcome_counts(output_dir: str, save_token: int) -> dict[str, int]:
    """Cached because the rollout panel is a fragment that reruns five times a second."""
    return count_outcomes(Path(output_dir))


def outcome_tally(output_dir: str) -> None:
    """Success rate over every judged episode already saved in this directory."""
    counts = _outcome_counts(output_dir, st.session_state.get("_saved_rollouts", 0))
    ...
    st.caption(
        f"📊 {rate} success over {judged} judged episode(s) in `{output_dir}` "
